...
**`deploy-scaleway` when:** **dev** and **staging** run on `push` to `main`, `feature/*`, or `fix/*`, and on `pull_request` (dev only) or matching `workflow_dispatch`. **prod** runs on `push` to a **`v*` tag** *or* **`push` to `main`** (so release merges that no longer trigger a separate tag-only pipeline still deploy prod), plus `workflow_dispatch` for prod.
