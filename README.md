...
Shared reusable GitHub Actions workflows and the **scaleway-secrets** composite action for brijyt-*-web, brijyt-*-api, and brijyt-*-mcp projects.
...
| [notify-deploy](.github/workflows/notify-deploy.yml) | verified-dev?, verified-staging?, verified-prod? | message-ts-staging, message-ts-prod | slack-bot-token |
| [notify-failure](.github/workflows/notify-failure.yml) | needs-json (optional, pass caller `toJSON(needs)`) | - | slack-bot-token |
| [notify-e2e](.github/workflows/notify-e2e.yml) | environment?, message-ts?, message-ts-staging?, e2e-conclusion, e2e-run-url? | - | slack-bot-token |
| [trigger-chat-e2e](.github/workflows/trigger-chat-e2e.yml) | environment? (choice: dev/staging/prod, default `staging`) | e2e_conclusion, e2e_run_url | e2e-trigger-app-private-key (+ caller var `E2E_TRIGGER_APP_CLIENT_ID`) |
...
**Post-deploy workflows:** `verify-deploy` polls `https://{app}-{env}.brijyt.io/health` when the repository name ends with `-api` or `-mcp`, otherwise `https://{app}-{env}.brijyt.ai/health`, until JSON **`testedSha`** strictly equals the workflow `github.sha`. When **`health-urls-json`** is set, only environments with a non-empty URL array in that JSON are verified (others are skipped — useful for Lovable prod-only deploys). Optional **`health-slugs-json`**: JSON array of slugs; for each active env, checks every `https://{slug}-api-{env}.{domain}/health` (same domain rule as default). Optional **`health-urls-json`**: explicit per-env URL lists (ignored when `health-slugs-json` is set). **Python** (`python-build-push-docker`): `testedSha` ← `APP_TESTED_SHA` (`github.sha`); `buildNumber` ← `APP_BUILD_NUMBER` (PR head on PR builds, otherwise `github.sha`). **Node** (`brijyt-chat-web`, `scripts/generate-health-assets.mjs`): `testedSha` ← `BUILD_TESTED_SHA`, `buildNumber` ← `BUILD_SOURCE_SHA` (set to `github.sha` in `node-build-push-docker`). Docker uses different arg names (`APP_*` vs `BUILD_*`), but on a normal **push** workflow **`buildNumber`** is the deployed app commit—the same SHA you see at `HEAD` in the repo for both API and web. Values differ mainly on **`pull_request`** workflows (merge ref vs PR head). `notify-deploy` and `notify-failure` use `slackapi/slack-github-action` to post to `#ci-cd`. Both resolve the Linear ticket from the branch name, commit message, or PR title automatically. The caller is responsible for `needs`/`if` conditions (especially `notify-failure`, which must list all upstream jobs). **`notify-deploy`** exposes **`message-ts-staging`** and **`message-ts-prod`** (Slack `ts` of the matching deploy messages) so callers can post E2E results as thread replies.

**Post-deploy E2E:** callers run **`trigger-chat-e2e`** with **`environment`** (`staging` default, or `prod` after a tag/`verified-prod`). It dispatches `brijyt-chat-e2e` workflow **`e2e.yml`**, polls until completion, and returns **`e2e_conclusion`** / **`e2e_run_url`**. Typical split: **staging** on the release-please PR (pre-merge); **prod only** on tag `v*` (staging+prod still redeploy; do not re-run staging E2E). Uses a **GitHub App** installation token (org-owned, not a personal PAT): org variable **`E2E_TRIGGER_APP_CLIENT_ID`**, org secret **`E2E_TRIGGER_APP_PRIVATE_KEY`** (pass as **`e2e-trigger-app-private-key: ${{ secrets.E2E_TRIGGER_APP_PRIVATE_KEY }}`**). Install the app on **`brijyt/brijyt-chat-e2e`** only with **Actions: Read and write**. Post the result with **`notify-e2e`** (`environment`, `message-ts` from the matching `notify-deploy` output; `message-ts-staging` remains a fallback). Example caller: `trigger_e2e` (staging if only staging verified, prod if `verified-prod`) → `notify_e2e`.
