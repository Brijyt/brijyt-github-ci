...
| [trigger-chat-e2e](.github/workflows/trigger-chat-e2e.yml) | - | e2e_conclusion, e2e_run_url | e2e-dispatch-token |
...
**Post-deploy E2E (staging):** after staging verify, callers can run **`trigger-chat-e2e`**, which dispatches `brijyt-chat-e2e` workflow **`e2e.yml`** (`environment: staging`), polls until completion, and returns **`e2e_conclusion`** / **`e2e_run_url`**. Pass secret **`e2e-dispatch-token: ${{ secrets.E2E_DISPATCH_TOKEN }}`** — a fine-grained PAT with **Actions: Read and write** on **`brijyt/brijyt-chat-e2e`** only. Add **`E2E_DISPATCH_TOKEN`** as an org or repo secret on each caller that opts in (e.g. `brijyt-agentic-reply-api`). Example caller jobs: `trigger_e2e` (needs staging verified) → `notify_e2e` (thread reply on `notify_deploy.outputs.message-ts-staging`).
